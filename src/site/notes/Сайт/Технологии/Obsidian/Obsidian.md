---
{"dg-publish":true,"dg-path":"Технологии/Obsidian/Obsidian.md","permalink":"/Технологии/Obsidian/Obsidian/","tags":["Obsidian"],"created":"2026-03-30T22:01:00","dg-note-properties":{"date":"2023-11-09","cssclasses":["note","hide-backlinks"],"aliases":["Обсидиан","#Obsidian","Обсидиану","Obsidian"],"Класс":"[[24. Страница на сайт|Страница]]","published":"2026-03-30T22:01:00","Статус страницы":["закончено"],"tags":["Obsidian"]}}
---


Obsidian — приложение для ведения заметок в формате Markdown.  

> [!bare] [![banner.png\|400x150](/img/user/_/attachments/banner.png)](https://obsidian.md) 

> [!bare] [![400](/img/user/_/attachments/banner.png)](https://obsidian.md) 

## Ссылки

- [Официальный сайт](https://obsidian.md)
- [Форум](https://forum.obsidian.md/)
- [Дискорд](https://discord.gg/obsidianmd)
- [Справка](https://help.obsidian.md/Home)
- [Русскоязычная справка](https://publish.obsidian.md/help-ru/Начните+здесь)
- [Документация для разработчиков](https://docs.obsidian.md/Home)
- [Что нового](https://obsidian.md/changelog/)
- [Дорожная карта](https://obsidian.md/roadmap/)

## Преимущества Obsidian

- Локальное хранение заметок:
    - безопасность
    - скорость
    - независимость от интеренета
- Открытый текстовый формат
    - надёжность
    - независимость от приложения
    - простая интеграция с другими инструментами
- Расширяемость плагинами
- Настраиваемый внешний вид
- Шаблоны
- Бесплатно (для личного пользования)

## Дополнительная информация

- Приложение Obsidian для десктопных компьютеров написано на основе Electron.
- Приложение для Android основано на  [Capacitor](https://capacitorjs.com/)
- [Obsidian 1.1.16](https://github.com/obsidianmd/obsidian-releases/releases/tag/v1.1.16) — последняя версия, которая работает на Windows 7/8
    - Из этого установщика можно обновиться до 1.6.7
